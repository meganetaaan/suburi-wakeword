...
The targeted hard-negative follow-up is recorded separately in `docs/memo/2026-05-13-expanded5k-hard-negative-iteration.md`. It reduced FAR/sample but hurt recall, so do not keep adding negative weight or hard-negative count. User feedback: `ハイスタックチャンネル` is close to acceptable, so the safer lexical-impostor profile is recorded in `docs/memo/2026-05-13-lexical-negative-profile.md` (`スタッキーちゃん`, `スタッフさん`, `スタックあんちゃん`, inserted/context words). The lexical run is better balanced than the first hard-negative run, but only modestly improves equal-recall FAR (`expanded-5k + negw1.25 @ 0.965`: recall `0.785`, FAR/sample `0.204`; `lexical + negw1.25 @ 0.955`: recall `0.785`, FAR/sample `0.197`). The substitution-only follow-up in `docs/memo/2026-05-13-substitution-negative-profile.md` reduced FAR more but did not preserve recall (`substitution + negw1.25 @ 0.875`: recall `0.695`, FAR/sample `0.083`). Use `expanded-5k + negative_class_weight=1.25` as the current operating candidate until a rebalanced profile clearly improves equal-recall FAR.
