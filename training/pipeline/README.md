...
The recommended first thousands-scale profile is `expanded-5k`. It expects four voice profiles and creates 912 base TTS jobs: 288 positive, 576 negative, and 48 English holdout. With the current raw + augmentation expansion this produces 6,384 utterances. Follow-up profiles add targeted negatives for experiments: `expanded-5k-hard-negatives` adds Japanese near-miss continuations from false-accept analysis, `expanded-5k-lexical-negatives` focuses on non-accept lexical impostors plus light contexts, and `expanded-5k-substitution-negatives` keeps only substitution impostors such as `スタッキーちゃん` and `スタックあんちゃん`. The hard-negative and substitution-only real CV runs lowered recall too much; the lexical run was safer but only modestly improved equal-recall FAR. Treat these profiles as rebalance experiments rather than the default dataset.
